...
export function runExpertAnalysis(rawContext?: Partial<ProjectContext>, rawIssue?: Partial<IssueInput>): CopilotAnalysisResult {
  ...
  const issue: IssueInput = {
    issueCategories: Array.isArray(rawIssue?.issueCategories) && rawIssue.issueCategories.length > 0
      ...
      : ['EMC'],
    requirement: rawIssue?.requirement || '符合车规级 CISPR 25 Class 5 辐射发射标准限值',
    actualMeasurement: rawIssue?.actualMeasurement || '150MHz 频点超标 +3.0 dB',
    testCondition: rawIssue?.testCondition || '13.5V 供电，额定负载工况',
    environment: rawIssue?.environment || '白盒测试暗室',
    failurePhenomenon: rawIssue?.failurePhenomenon || '在极端工况或摸底测试下参数超出允许裕量',
    engineeringConcern: rawIssue?.engineeringConcern || '改板周期击穿 14 天交付节点',
  ...
  const isBldc = cats.includes('BLDC Motor Drive') || allText.includes('bldc') || allText.includes('电机') || allText.includes('泵升') || allText.includes('米勒') || allText.includes('snubber');
  const isEmc = cats.includes('EMC') || allText.includes('cispr') || allText.includes('emc') || allText.includes('辐射') || allText.includes('传导');
  const isComponent = cats.includes('Component Alternative') || allText.includes('mosfet') || allText.includes('替代') || allText.includes('换料');
  const isWcca = cats.includes('WCCA') || allText.includes('error') || allText.includes('wcca') || allText.includes('裕量') || allText.includes('公差');
