...
import {
  ...
  SAMPLE_SAFETY_TRACEABILITY_CHAIN,
  SAMPLE_FMEDA_ROWS,
...
export const FunctionalSafetyReliabilityView: React.FC = () => {
  ...
  // FMEDA 数据与计算
  const fmedaMetrics = calculateFmedaMetrics(SAMPLE_FMEDA_ROWS);

  // 电容寿命交互参数
  const [capParams, setCapParams] = useState({
    nominalHours: 5000,
    ratedTempC: 105,
    operatingTempC: 85,
    rippleOperatingA: 3.2,
    rippleRatedA: 4.5,
  });
  ...
  const secondSourceResult = compareSecondSource('Infineon BSC035N04LS', 'ON Semi NTMFS4C029N');
  const pcnResult = evaluatePcn('WAFER_FAB');
  const esdResult = evaluateEsdProtection();
  const bciResult = evaluateBciImmunity();
      ...
      {activeSubTab === 'HARA_TRACE' && (
        <div className="space-y-4">
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
            ...
            <div className="space-y-4">
              {SAMPLE_SAFETY_TRACEABILITY_CHAIN.map((node, idx) => (
                <div
                  key={idx}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
              ...
              <div className="mt-2 text-[11px] text-emerald-300 flex items-center gap-1">
                <CheckCircle2 className="w-3.5 h-3.5" /> 满足 ASIL C 标准要求
              </div>
            </div>
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-sm font-bold text-white flex items-center gap-2">
                ...
                <span>逆变器直通起火 FTA 故障树 (Top Event Fault Tree)</span>
              </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-3">
            ...
            <div className="bg-slate-950 p-3 rounded-lg border border-slate-800 text-xs space-y-2 text-slate-300">
              ...
              <div>• 滤波措施: {bciResult.filteringMeasures.join('；')}</div>
            </div>
          </div>
        </div>
      )}
