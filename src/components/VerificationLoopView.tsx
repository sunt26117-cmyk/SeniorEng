...
export const VerificationLoopView: React.FC<VerificationLoopViewProps> = ({
  ...
  const nextBestAction = generateNextBestAction(daysRemaining);
  const voiTests = calculateVoiTestPriorities();
  const verificationPlan = generateStructuredVerificationPlan();
  ...
  const [currentRisk, setCurrentRisk] = useState<TransparentRiskScore>({
    ...
    confidence: 'LOW',
    uncertainty: 'HIGH',
    verificationGap: 'HIGH',
    majorRiskDriver: '3800rpm 急停母线泵升实测未知 + 米勒直通风险未在真实台架闭环',
  });

  // 实测数据回填表单
  const [testForm, setTestForm] = useState<TestResultEntry>({
    testId: 'TEST-2026-09-01',
    condition: '电机 3800rpm 急停，三相下桥短接制动使能',
    instrument: 'Tektronix MSO 54 (1GHz) + IsoVu 差分光隔离探头',
    measurement: '母线泵升最高峰值 Vbus_peak & 门极 Vgs_peak',
    result: 'Vbus_peak = 16.2V, Vgs_peak = 0.65V (无直通)',
    passFail: 'PASS',
  ...
  const [decisionHistory, setDecisionHistory] = useState<DecisionRecord[]>([
    {
      ...
      problemSummary: '3800rpm BLDC 急停时母线预估泵升达 39.1V，逼近 40V 耐压上限',
      optionsConsidered: ['方案 A: TVS 硬件钳位', '方案 B: 软件全下桥短接能耗制动', '方案 C: 改板换 60V MOS'],
      chosenOption: '方案 B: 软件全下桥短接能耗制动 + 门极有源米勒钳位',
      justification: '成本增量为 0，开发周期仅 3 天保住 15 天节点，从物理源头切断能量回馈',
      rejectedOptionsReason: {
        '方案 A': 'TVS 在 3800rpm 持续浪涌下发热烧毁且 38.9V 残压缺乏安全裕量',
        '方案 C': '需重新投样改板 3~4 周，违背 15 天交付硬红线',
      },
      verificationPlan: '台架光隔离示波器实测 Vbus <= 18.0V 判定通过',
      owner: '张工 & 李工',
      status: 'OPEN',
    },
  ]);
