...
import {
  ...
  evaluateComponentChangeImpact,
} from '../data/designReviewEngine';
import {
  GOLD_STANDARD_CASES,
  runGoldStandardCaseRegression,
} from '../data/goldStandardCases';
import { PhaseCheckItem, ComponentChangeImpactItem } from '../types';
...
export const DesignReviewRegressionView: React.FC<DesignReviewRegressionViewProps> = ({
  ...
  // 评审阶段
  const [selectedPhase, setSelectedPhase] = useState<'Concept' | 'EVT' | 'DVT' | 'PVT' | 'SOP'>('DVT');
  const reviewChecklist = getPhaseReviewChecklist(selectedPhase);

  // 最坏情况候选
  const worstCases = generateWorstCaseCandidates();

  // 器件变更分析
  const [selectedComponent, setSelectedComponent] = useState<
    ComponentChangeImpactItem['componentCategory']
  >('MOSFET');
  const changeImpact = evaluateComponentChangeImpact(selectedComponent);

  // 回归测试用例状态
  const [regressionResults, setRegressionResults] = useState<Record<string, 'PASS' | 'FAIL'>>({});
  const [activeCaseId, setActiveCaseId] = useState<string>('Case02');
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-slate-300 font-semibold">
              3800rpm BLDC 急停母线泵升 37.8V ｜ MOSFET 40V ｜ 距节点 15 天
            </span>
          </div>
          <p className="text-xs text-slate-400">
            一键载入标准验收用例，端到端执行 P001 识别、动能计算、Failure Chain、三方案权衡与 Next Best Action。
          </p>
        </div>
        {onLoadScenarioSection14 && (
          <button
